...
export const CreateProductRequestSchema = z.object({
  name: z.string().trim().min(1, '商品名称不能为空').max(64, '商品名称长度不能超过 64 个字符').describe('名称'),
  brief: z.string().trim().max(64, '简介长度不能超过 64 个字符').optional().describe('简介'),
  description: z.string().trim().max(500, '描述长度不能超过 500 个字符').optional().describe('描述'),
...
export const UpdateProductRequestSchema = z.object({
  name: z.string().trim().max(64, '商品名称长度不能超过 64 个字符').optional().describe('名称'),
  brief: z.string().trim().max(64, '简介长度不能超过 64 个字符').optional().describe('简介'),
  description: z.string().trim().max(500, '描述长度不能超过 500 个字符').optional().describe('描述'),
  price: z.number().min(0, '价格不能为负数').max(999999, '价格不能超过 999999').optional().describe('价格'),
  status: z.number().optional().describe('状态（0=全局下架 1=全局上架）'),
