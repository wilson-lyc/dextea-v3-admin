...
export default function CustomizationOptionBindingPanel({ ingredientId, unit }: CustomizationOptionBindingPanelProps) {
  ...
  const handleBind = async () => {
    ...
    setBinding(true)
    try {
      const res = await bindOptionToIngredient(ingredientId, optionId, Number(bindQuantity) || 0)
      if (res.code === 0) {
  ...
  const handleEditQuantity = async () => {
    ...
    setEditing(true)
    try {
      const res = await updateIngredientOptionQuantity(ingredientId, editOption.optionId, Number(editQuantity) || 0)
      if (res.code === 0) {
        toast.success(res.message)
        setEditOpen(false)
        setEditOption(null)
